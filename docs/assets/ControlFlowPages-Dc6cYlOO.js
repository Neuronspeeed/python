import{c as e,r as t}from"./index-Dfa-XKD4.js";const n=[{section:`Core Concepts`,signature:`Dynamic Typing`,description:`Python tracks object types at runtime—no manual type declarations needed. Variables are just names bound to objects.`,complexity:`Concept`,example:`x = 42        # x refers to an int
...
len([1, 2, 3])  # 3 (list length)`},{section:`Why & When`,signature:`Choosing Numeric Types`,description:`int for exact counting, float for measurements, Decimal for money/precision, Fraction for exact ratios.`,complexity:`Concept`,example:`# INT - counting, indexing, exact arithmetic
count = 42
index = arr[5]
...
def modify(lst):
    ...
    lst.append(99)`},{section:`Object Hierarchy`,signature:`Programs → Modules`,description:`Programs are composed of modules (.py files). Each module is a namespace containing definitions.`,complexity:`Concept`,example:`# my_module.py
def greet(name):
    return f"Hello, {name}"
...
def foo(): pass
type(foo)  # <class 'function'>`},{section:`Mutability`,signature:`Immutable Objects`,description:`Cannot be changed after creation. Includes: int, float, str, tuple, frozenset, bytes. Safe as dict keys.`,complexity:`Concept`,example:`s = "hello"
...
d = {[1, 2]: "x"}  # TypeError: unhashable type`},{section:`Type Categories`,signature:`Numbers`,description:`int (unlimited precision), float (64-bit), complex, Decimal (exact), Fraction. Support +, -, *, /, //, %, **.`,complexity:`Concept`,example:`42                    # int
3.14                  # float
2 + 3j                # complex
...
{1, 2} - {2, 3}      # {1} (difference)`},{section:`Numeric Literals`,signature:`Alternative Bases`,description:`Integers can be written in hex (0x), octal (0o), and binary (0b). Underscores improve readability.`,complexity:`Concept`,example:`0xFF        # 255 (hexadecimal)
0o77        # 63 (octal)
0b1010      # 10 (binary)
...
from fractions import Fraction
Fraction(1, 3) + Fraction(1, 6)  # Fraction(1, 2)`},{section:`Operators & Expressions`,signature:`Operator Precedence`,description:`** binds tightest, then */%, then +-. Use parentheses to override. Associativity: ** is right-to-left.`,complexity:`Concept`,example:`2 + 3 * 4     # 14 (not 20)
(2 + 3) * 4   # 20
...
8 >> 2        # 2  (shift right = divide by 2^n)`},{section:`Numeric Modules`,signature:`math Module`,description:`Standard math functions: sqrt, ceil, floor, log, sin, cos, pi, e, factorial, gcd.`,complexity:`Concept`,example:`import math
math.sqrt(16)     # 4.0
math.ceil(3.2)    # 4
math.floor(3.8)   # 3
...
stats.mode(data)    # 2
stats.stdev(data)   # 1.472...`},{section:`Philosophy & Tools`,signature:`The Zen of Python`,description:`Python's design principles. "Beautiful is better than ugly. Simple is better than complex."`,complexity:`Concept`,example:`import this  # Print the Zen of Python
...
#  you probably don't."`}],r=[{section:`Assignment Fundamentals`,signature:`target = object`,description:`Basic assignment creates a reference to an object. Variables are names bound to objects, not containers holding values.`,complexity:`Concept`,example:`x = 42          # x refers to int object 42
...
with open("f") as f:  # assigns 'f' to file handle
    pass`},{section:`Why & When`,signature:`Basic vs Sequence Assignment`,description:`Use sequence assignment for unpacking, swapping, or multiple return values. Use basic for single values.`,complexity:`Concept`,example:`# BASIC ASSIGNMENT - single values
x = 42
name = "Alice"
is_valid = True
...
# RULE: Never use mutable defaults or multiple assignment for mutables`},{section:`Sequence Assignment`,signature:`a, b = iterable`,description:`Tuple unpacking assigns elements by position. Left side can be tuple or list of targets.`,complexity:`Concept`,example:`# Basic unpacking
a, b = 1, 2         # a=1, b=2
...
a, *b = [1]  # a=1, b=[]`},{section:`Multiple & Augmented`,signature:`a = b = c = value`,description:`Multiple-target assignment. All names reference the same object. Safe for immutables, risky for mutables.`,complexity:`Concept`,example:`# Safe with immutables
a = b = c = 0
a = 1         # only 'a' changes
print(b, c)   # 0 0
...
print(M)        # [1, 2] — M unchanged`},{section:`Walrus Operator`,signature:`name := expression`,description:`Named expression (Python 3.8+). Assigns and returns value in one expression. Useful in conditions.`,complexity:`Concept`,example:`# In while loops - read, assign, test
while (line := input()) != "quit":
...
[clean for s in strings if (clean := s.strip())]`},{section:`Variable Naming`,signature:`Naming Rules`,description:`Names must start with letter or underscore, followed by letters, digits, or underscores. Case-sensitive.`,complexity:`Concept`,example:`# Valid names
count = 1
_private = 2
myVar2 = 3
CamelCase = 4
...
for _ in range(3):  # don't need loop variable
    print("hi")`},{section:`Expression Statements`,signature:`Expression as Statement`,description:`Any expression can be a statement. Common for function calls with side effects.`,complexity:`Concept`,example:`# Function calls as statements
...
# Same issue: reverse(), clear(), extend()`},{section:`Print Operations`,signature:`print(*objects, sep, end, file)`,description:`Prints objects to stream. sep separates items (default space), end terminates (default newline).`,complexity:`Concept`,example:`print("a", "b", "c")          # a b c
print("a", "b", sep="-")      # a-b
...
sys.stdout.close()
sys.stdout = original
print("Back to console")    # normal`}],i=[{section:`Fundamentals`,signature:`if statement basics`,description:`Execute code ONLY if condition is True. Must end with colon. Indented block runs when True, skipped when False.`,complexity:`Concept`,example:`# Basic if - runs ONLY when True
if 2 + 2 == 4:
...
# elif grade >= 80:  # Correct`},{section:`if Statement`,signature:`if test: ... elif: ... else:`,description:`Primary selection tool. Evaluates tests top-to-bottom, executes first true block, then exits.`,complexity:`Concept`,example:`x = 85
if x >= 90:
    grade = 'A'
elif x >= 80:
    grade = 'B'
...
if age >= 18:
    ...
    print("Too young")
# Output: Can drive`},{section:`Ternary Expression`,signature:`Y if X else Z`,description:`One-line conditional expression. Returns Y if X is true, else Z. Not a statement.`,complexity:`Concept`,example:`x = 10
...
if x < 3:
    ...
    size = "large"`},{section:`Dictionary Branching`,signature:`dict[key] for branching`,description:`Use dict to map choices to values or functions. More flexible than long if/elif chains.`,complexity:`O(1)`,example:`# Value mapping
day_names = {
...
print(ops['+'](3, 4))  # 7`},{section:`Truth Values`,signature:`Falsy Values`,description:`Values that evaluate to False: None, False, 0, 0.0, "", [], {}, set(), range(0).`,complexity:`Concept`,example:`# All falsy
bool(None)    # False
bool(False)   # False
bool(0)       # False
bool(0.0)     # False
...
if value is not None:  # explicit None check
    process(value)`},{section:`Boolean Operators`,signature:`x and y`,description:`Returns x if x is falsy, otherwise y. Short-circuits: stops if first is false.`,complexity:`O(1)`,example:`# Short-circuit evaluation
True and True    # True
...
if lst and lst[0] > 10:  # safe: checks lst first
    process(lst)`},{section:`Syntax Rules`,signature:`Indentation-Based Blocks`,description:`Python uses indentation (not braces) to define blocks. Consistent spaces required.`,complexity:`Concept`,example:`# Correct indentation
if True:
    print("a")
    print("b")    # same block
    if True:
...
help(greet)           # shows docstring`}],a=[{section:`Performance & Patterns`,signature:`if-elif vs dict dispatch`,description:`Dict dispatch is O(1) for value lookup. if-elif is O(n) worst case. Use dict for many branches with simple mapping.`,complexity:`Concept`,example:`# IF-ELIF CHAIN - O(n) worst case
day_num = 4
if day_num == 0:
    day = 'Monday'
elif day_num == 1:
...
# RULE: If it doesn't fit on one readable line, use if-else`}],o=[{section:`Fundamentals`,signature:`match subject: case pattern:`,description:`Pattern matching (Python 3.10+). Compares subject against patterns, executes first match. Structural pattern matching.`,complexity:`Concept`,example:`command = "quit"
match command:
    case "start":
        print("Starting...")
    case "stop":
...
match Point(x=10, y=20):
    ...
    case Point(x=x, y=0):
        print(f"X-axis: {x}")
    case Point(x=x, y=y):
        print(f"({x}, {y})")  # matches`}],s=[{section:`Fundamentals`,signature:`Why use loops?`,description:`Loops repeat code multiple times. Use while for condition-based repetition, for for iterating over collections.`,complexity:`Concept`,example:`# Loops repeat code - computers never get tired!
...
# In comprehension
results = [y for x in data if (y := process(x)) is not None]`}],c=[{section:`Fundamentals`,signature:`Comprehension Syntax`,description:`Comprehensions provide concise syntax for creating collections. Four types: list [], dict {k:v}, set {}, generator (). All share the same structure.`,complexity:`Concept`,example:`# List comprehension - builds entire list in memory
...
print(d)  # {0: 'apple', 1: 'banana', 2: 'cherry'}`}],l=[{section:`Fundamentals`,signature:`What is a function?`,description:`Functions are VALUES, like numbers and strings. They break code into reusable chunks. Call with () to execute.`,complexity:`Concept`,example:`# Functions are values!
...
# - Both!`},{section:`Why & When`,signature:`When to use functions`,description:`Use functions for code reuse, abstraction, and organization. Rule of thumb: if you copy-paste code, make it a function.`,complexity:`Concept`,example:`# WITHOUT FUNCTIONS - repetitive
...
# Only inline if profiling shows bottleneck`},{section:`Function Definition`,signature:`def func_name(params):`,description:`Defines a function with optional parameters. Functions are first-class objects.`,complexity:`O(1)`,example:`def greet(name):
    return f"Hello, {name}!"
...
print(result)  # (1, 3, 6)`},{section:`Parameters`,signature:`def func(a, b, c):`,description:`Positional parameters. Must be provided in order when calling.`,complexity:`O(1)`,example:`def greet(first, last, greeting):
...
def f(a, /, b, *, c):
    return a + b + c`},{section:`*args and **kwargs`,signature:`*args`,description:`Captures variable positional arguments as a tuple.`,complexity:`O(n)`,example:`def sum_all(*args):
    return sum(args)
...
def wrapper(func, *args, **kwargs):
    ...
    print("After call")
    return result`},{section:`Argument Unpacking`,signature:`func(*iterable)`,description:`Unpacks iterable as positional arguments when calling.`,complexity:`O(n)`,example:`def add(a, b, c):
    return a + b + c
...
def func(a, b, c, d):
    return a + b + c + d
print(func(*args, **kwargs))  # 10`},{section:`Lambda Functions`,signature:`lambda args: expr`,description:`Anonymous function for simple expressions. Returns result of expression.`,complexity:`O(1)`,example:`square = lambda x: x ** 2
print(square(5))  # 25
...
print(squares)  # [1, 4, 9, 16, 25, 36]`},{section:`Scopes & LEGB`,signature:`LEGB Rule`,description:`Name lookup order: Local → Enclosing → Global → Built-in. First match wins.`,complexity:`Concept`,example:`x = "global"          # Global scope
...
# Use closure or global to persist state`},{section:`First-Class & Polymorphism`,signature:`First-class functions`,description:`Functions are objects—assign to variables, store in collections, pass as arguments.`,complexity:`Concept`,example:`# Assign to variable
...
print(total({1, 2, 3}))    # 6 (set)`},{section:`Recursion Patterns`,signature:`Cycle detection`,description:`Track visited items to avoid infinite loops in cyclic/graph structures.`,complexity:`O(n)`,example:`def traverse(node, visited=None):
    if visited is None:
        visited = set()
...
def dfs(graph, start, visited=None):
    ...
    for neighbor in graph[start]:
        if neighbor not in visited:
            dfs(graph, neighbor, visited)`}],u=[{section:`Decorators`,signature:`@decorator`,description:`Wraps function to modify its behavior. Applied from bottom to top.`,complexity:`O(1)`,example:`def uppercase(func):
...
print(d.year, d.month, d.day)  # 2024 1 15`}],d=[{section:`Closures`,signature:`Closure`,description:`Function that captures variables from enclosing scope.`,complexity:`O(1)`,example:`def make_multiplier(n):
    def multiplier(x):
...
def bad_modify():
    x = 20  # Creates new local x, doesn't modify global`},{section:`Higher-Order Functions`,signature:`map(func, iterable)`,description:`Applies function to each element. Returns iterator.`,complexity:`O(n)`,example:`nums = [1, 2, 3, 4, 5]
...
def get_pair() -> tuple[int, str]:
    return (1, "one")`}],f=[...l,...u,...d],p=[{signature:`Why use OOP?`,description:`OOP organizes code into reusable, modular units. Classes bundle data + behavior together, making code easier to understand, maintain, and extend.`,complexity:`Concept`,section:`Why & When`,example:`# Without OOP - scattered functions and data
name = "Alice"
balance = 100
...
# Rule: Mix-ins OK, inheritance trees → composition`},{signature:`Attribute search tree`,description:`When accessing obj.attr, Python searches: instance → class → superclasses (left-to-right, bottom-to-top). First match wins.`,complexity:`Concept`,section:`Inheritance`,example:`class A:
    x = "from A"
...
# Do: if hasattr(obj, '__iter__'):`},{signature:`@property`,description:`Define getter method accessed like an attribute. Encapsulates internal state. Computed properties without method call syntax.`,complexity:`O(1)`,section:`Properties`,example:`class Circle:
    def __init__(self, radius):
        self._radius = radius
...
t.fahrenheit = 100
print(t.celsius)     # 37.77...`},{signature:`@classmethod`,description:`Method that receives class (cls) instead of instance. Use for alternative constructors or operations on class state.`,complexity:`O(1)`,section:`Class & Static Methods`,example:`class Date:
    def __init__(self, year, month, day):
        self.year = year
        self.month = month
        self.day = day
...
# @classmethod - need class for alternative constructors`},{signature:`__dict__, __class__, __bases__`,description:`Namespaces are dicts. Instances link to class via __class__, classes link to parents via __bases__.`,complexity:`O(1)`,section:`Namespace Internals`,example:`class Parent:
    x = 1
...
class Employee(Person):
    def greet(self):
        ...
        return f"{base}, an employee"`},{signature:`4 ways to specialize`,description:`Inherit (use as-is), Override (replace), Extend (call super + add), Provide (subclass implements).`,complexity:`Concept`,section:`Specialization Patterns`,example:`class Base:
    def inherit_me(self):
        return "base"
...
class Child(Base):
    ...
    def action(self):
        return "child action"`}],h=[{signature:`S - Single Responsibility`,description:`A class should have only ONE reason to change. Each class handles one job. Split large classes into focused ones.`,complexity:`Principle`,section:`SOLID Principles`,example:`# BAD: One class doing everything
class User:
    def __init__(self, name, email):
        self.name = name
        self.email = email
...
# Avoid when: "this object does many things"`},{signature:`__str__(self)`,description:`Human-readable string representation. Used by print() and str(). User-friendly output.`,complexity:`O(1)`,section:`Dunder Methods`,example:`class Person:
    def __init__(self, name, age):
        self.name = name
        self.age = age
...
help(Calculator)  # Shows formatted docs`}],_=[{signature:`ABC and @abstractmethod`,description:`Define abstract classes that cannot be instantiated. Force subclasses to implement specific methods.`,complexity:`O(1)`,section:`Abstract Classes`,example:`from abc import ABC, abstractmethod
...
rect = Rectangle(5, 3)
print(rect.area())       # 15
print(rect.perimeter())  # 16`},{signature:`@dataclass`,description:`Auto-generate __init__, __repr__, __eq__ and more. Reduces boilerplate for data-holding classes.`,complexity:`O(1)`,section:`Dataclasses`,example:`from dataclasses import dataclass, field
...
# When run directly: python person.py → runs tests`}],v=[{signature:`Singleton Pattern`,description:`Ensure only one instance exists. Use __new__ or module-level instance. Consider: do you really need this?`,complexity:`O(1)`,section:`Design Patterns`,example:`class Singleton:
    _instance = None
...
# Use metaclass for framework-level control`}],y=[...p,...m,...h,...g,..._,...v];var b=e(),x=`Python's Object Model Foundation
...
\`\`\`
`;function S(){return(0,b.jsx)(t,{type:`Python Fundamentals`,badge:`py`,color:`var(--accent-functions)`,description:`Core concepts: dynamic typing, strong typing, polymorphism. Understanding Python's object model and type categories.`,intro:x,methods:n})}var C=`Assignment Creates References, Not Copies
...
\`\`\`
`;function w(){return(0,b.jsx)(t,{type:`Statements`,badge:`=`,color:`var(--accent-statements)`,description:`Assignment forms, variable naming, expression statements, and print operations. The building blocks of Python programs.`,intro:C,methods:r})}var T=`Guard Clauses and Early Returns
...
\`\`\`
`;function E(){return(0,b.jsx)(t,{type:`Conditionals`,badge:`if`,color:`var(--accent-none)`,description:`Selection and branching with if, ternary expressions, and boolean logic. Dictionary dispatch for cleaner multi-way branching.`,intro:T,methods:i})}var D=`Dictionary Dispatch for O(1) Lookups
...
\`\`\`
`;function O(){return(0,b.jsx)(t,{type:`Selection Patterns`,badge:`O(1)`,color:`var(--accent-none)`,description:`Performance comparisons and best practices: if-elif vs dict vs match, short-circuit evaluation, when to use ternary.`,intro:D,methods:a})}var k=`Pattern Matching for Structural Destructuring
...
if sys.version_info >= (3, 10):
    ...
    if value == pattern: ...
\`\`\`
`;function A(){return(0,b.jsx)(t,{type:`Match Statement`,badge:`match`,color:`var(--accent-none)`,description:`Structural pattern matching (Python 3.10+). Destructure sequences, match types, bind variables, use guards. More powerful than switch.`,intro:k,methods:o})}var j=`For Loops: Python's Preferred Iteration
...
\`\`\`
`;function M(){return(0,b.jsx)(t,{type:`Loops`,badge:`for`,color:`var(--accent-none)`,description:`Python loops: for iterates over sequences, while repeats until condition is false. Includes iteration tools and loop control.`,intro:j,methods:s})}var N=`List vs Generator: The Memory Trade-Off
...
\`\`\`
`;function P(){return(0,b.jsx)(t,{type:`Comprehensions`,badge:`[]`,color:`var(--accent-none)`,description:`Concise syntax for creating collections: list, dict, set, generator. Transform and filter iterables in one readable expression.`,intro:N,methods:c})}var F=`Functions as First-Class Objects
...
say_hi()  # Call #2 
 Hi!
\`\`\`python
`;function I(){return(0,b.jsx)(t,{type:`Functions`,badge:`def`,color:`var(--accent-functions)`,description:`Functions are first-class objects in Python. Use def for named functions, lambda for anonymous functions.`,intro:F,methods:f})}var L=`Classes and Objects Fundamentals
...
\`\`\`python
`;function R(){return(0,b.jsx)(t,{type:`OOP`,badge:`class`,color:`var(--accent-oop)`,description:`Object-Oriented Programming in Python. Classes bundle data + behavior. Use when you have state + multiple operations on that state.`,intro:L,methods:y})}export{P as ComprehensionsPage,O as ConditionalPatternsPage,E as ConditionalsPage,I as FunctionsPage,S as FundamentalsPage,M as LoopsPage,A as MatchPage,R as OOPPage,w as StatementsPage};