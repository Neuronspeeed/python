import{c as e,r as t}from"./index-Dfa-XKD4.js";const n={string:{type:`String`,badge:`str`,color:`var(--accent-str)`,description:`Immutable text sequences. Every operation returns a NEW string. Use join() for building, f-strings for formatting.`,intro:`String Building Performance: Choose the Right Tool
...
# Reassign to change
name = name.upper()    # Now name is "PICARD"`},{section:`Why & When`,signature:`Why use Strings?`,description:`Strings store text data. They are IMMUTABLE - every operation creates a new string. Use for text processing, user input, file paths, API responses.`,complexity:`Concept`,example:`# Strings are IMMUTABLE sequences of characters
name = "Alice"
...
"%s" % x       # %-formatting
"{}".format(x) # .format()`},{section:`Case Methods`,signature:`str.upper()`,description:`Returns a copy with all characters converted to uppercase.`,complexity:`O(n)`,example:`s = "hello"
...
print(s.swapcase())  # "hELLO wORLD"`},{section:`Case Methods`,signature:`str.casefold()`,description:`Returns casefolded copy for caseless matching. More aggressive than lower().`,complexity:`O(n)`,example:`print("ß".casefold())  # "ss"`},{section:`Checking Methods`,signature:`str.islower()`,description:`Returns True if all cased characters are lowercase.`,complexity:`O(n)`,example:`print("hello".islower())  # True
...
print(s.endswith(".py"))  # True`},{section:`Strip Methods`,signature:`str.strip([chars])`,description:`Returns copy with leading/trailing characters removed.`,complexity:`O(n)`,example:`s = "  hello  "
...
print(s.rstrip())  # "hello"`},{section:`Split & Join`,signature:`str.split(sep=None, maxsplit=-1)`,description:`Returns list of words using sep as delimiter.`,complexity:`O(n)`,example:`s = "a,b,c"
...
# ROT13 cipher
import codecs
codecs.encode("hello", "rot13")  # "uryyb"`},{section:`Padding & Alignment`,signature:`str.center(width[, fillchar])`,description:`Returns centered string of given width.`,complexity:`O(n)`,example:`print("hi".center(10, "-"))  # "----hi----"`},{section:`Padding & Alignment`,signature:`str.ljust(width[, fillchar])`,description:`Returns left-justified string of given width.`,complexity:`O(n)`,example:`print("hi".ljust(5))  # "hi   "`},{section:`Padding & Alignment`,signature:`str.rjust(width[, fillchar])`,description:`Returns right-justified string of given width.`,complexity:`O(n)`,example:`print("42".rjust(5, "0"))  # "00042"`},{section:`Padding & Alignment`,signature:`str.zfill(width)`,description:`Returns string padded with zeros on the left.`,complexity:`O(n)`,example:`print("42".zfill(5))  # "00042"`},{section:`User Input`,signature:`input([prompt])`,description:`Reads a line from user input and returns it as a string. Optional prompt is displayed to user.`,complexity:`O(n)`,example:`# Basic input
...
print(num_int + 5)  # OK now`},{section:`Number Formatting`,signature:`f-string number format`,description:`Format numbers in f-strings using {value:format}. Use .Nf for decimals, , for thousands, % for percentages.`,complexity:`O(n)`,example:`# Fixed-point notation (N decimal places)
n = 7.125
...
# NFD: decomposed (macOS filenames)`}],i=[{section:`Fundamentals`,signature:`Integer Basics`,description:`Integers are whole numbers. Python has UNLIMITED size (arbitrary precision). Use underscores for readability.`,complexity:`Concept`,example:`# Integer literals
x = 1
...
print(round(3.5))   # 4
print(round(2.675, 2))  # 2.67 (float precision!)`}],a=[{section:`Fundamentals`,signature:`Float Basics`,description:`Floating-point numbers represent real numbers with decimals. IEEE 754 standard. Use E-notation for very large/small values.`,complexity:`Concept`,example:`# Float literals
x = 1.0
y = 1.25
z = -2.75
...
print(round(2.675, 2))   # 2.67 (not 2.68 due to binary storage!)`},{section:`Why & When`,signature:`Why use Floats?`,description:`Use floats for scientific calculations, measurements, continuous values. NOT for money (use Decimal). Understand precision limits.`,complexity:`Concept`,example:`# GOOD uses for float:
...
for i in range(10):
    total += Decimal('0.1')
print(total)  # 1.0 (exact!)`},{section:`Float Methods`,signature:`float.is_integer()`,description:`Returns True if the float is finite with integral value (no decimal part).`,complexity:`O(1)`,example:`print((3.0).is_integer())   # True
...
c = 3 + 4j
print(c.conjugate())  # (3-4j)`},{section:`Float Properties`,signature:`float.real`,description:`Read-only property returning the real part (the float itself). Compatibility with complex.`,complexity:`O(1)`,example:`x = 3.14
print(x.real)  # 3.14
...
# Complex compatibility
c = 3 + 4j
print(c.imag)  # 4.0`},{section:`Arithmetic Operations`,signature:`x + y`,description:`Addition. Returns float if either operand is float. Watch for precision errors.`,complexity:`O(1)`,example:`print(3.5 + 2.5)   # 6.0
...
print((-1.0) ** 0.5) # (6.123233995736766e-17+1j) (complex!)`},{section:`Built-in Functions`,signature:`abs(x)`,description:`Returns absolute value (distance from zero).`,complexity:`O(1)`,example:`print(abs(-3.14))  # 3.14
print(abs(3.14))   # 3.14
print(abs(0.0))    # 0.0
...
# Equivalent to:
q, r = 7.5 // 2.0, 7.5 % 2.0
print(q, r)  # 3.0 1.5`},{section:`Math Module - Rounding`,signature:`math.floor(x)`,description:`Returns largest integer <= x (rounds toward -∞). Returns int in Python 3.`,complexity:`O(1)`,example:`import math
...
print(int(3.7))   # 3
print(int(-3.7))  # -3`},{section:`Math Module - Power & Logarithm`,signature:`math.sqrt(x)`,description:`Returns square root of x. Raises ValueError for negative x.`,complexity:`O(1)`,example:`import math
...
# Check if power of 2
def is_power_of_2(n):
    return n > 0 and math.log2(n).is_integer()`},{section:`Math Module - Special Values`,signature:`math.isnan(x)`,description:`Returns True if x is NaN (Not a Number). NaN != NaN, so use this function.`,complexity:`O(1)`,example:`import math
nan = float("nan")
...
print(math.isclose(0.0, 1e-10, abs_tol=1e-9))  # True`},{section:`Math Module - Trigonometry`,signature:`math.sin(x)`,description:`Returns sine of x (in radians). Returns value in [-1, 1].`,complexity:`O(1)`,example:`import math
...
# print(math.tan(math.pi / 2))  # 1.633123935319537e+16 (huge!)`}],o=[{section:`Fundamentals`,signature:`Boolean Basics`,description:`Bool type has only TWO values: True and False (capitalized!). Result of all comparisons and logical operations. Foundation of conditional logic.`,complexity:`Concept`,example:`# Boolean type has only 2 values
...
class MyClass:
    pass
print(bool(MyClass()))  # True`},{section:`Why & When`,signature:`Why use Booleans?`,description:`Booleans control program flow. Use for conditions, flags, validation, filtering. Leverage truthiness for cleaner code.`,complexity:`Concept`,example:`# Control flow - if/while/for conditions
is_valid = True
...
if cheap_condition and expensive_condition:
    pass  # Evaluate cheap first to short-circuit`},{section:`Comparison Operators`,signature:`== != < > <= >=`,description:`Compare values: == (equal), != (not equal), < > <= >= (ordering). Returns True or False. Common mistake: = vs ==`,complexity:`O(1)`,example:`# Equality
print(1 == 1)       # True
print(1 != 2)       # True
print("a" == "a")   # True
...
# More readable than:
print((1 < x) and (x < 10))  # Same but verbose`},{section:`Logical Operators`,signature:`and`,description:`Logical AND. Returns x if x is falsy, otherwise returns y. Short-circuits (stops if x is False).`,complexity:`O(1)`,example:`# Boolean results
...
print(not not [])  # False (not True)`},{section:`Identity & Equality`,signature:`x == y`,description:`Equality comparison. Returns True if x equals y. Compares VALUES.`,complexity:`O(1)`,example:`print(True == True)   # True
...
print(x is None)  # Pythonic way`},{section:`Boolean Arithmetic`,signature:`Boolean as Integer`,description:`Boolean is a subclass of int. True=1, False=0. Use sum() to count True values.`,complexity:`O(1)`,example:`# Arithmetic operations
print(True + True)    # 2
print(True * 10)      # 10
print(False + 5)      # 5
print(True - 1)       # 0
...
options = ["No", "Yes"]
choice = True
print(options[choice])  # "Yes" (index 1)`},{section:`Short-circuit Patterns`,signature:`Short-circuit evaluation`,description:`and/or stop evaluating as soon as result is determined. Prevents errors and improves performance.`,complexity:`O(1)`,example:`# Safe dictionary access (prevents KeyError)
d = {}
...
result = try_fast() or try_slow() or default_value`},{section:`Aggregate Functions`,signature:`all(iterable)`,description:`Returns True if ALL elements are truthy (or iterable is empty). Short-circuits at first False.`,complexity:`O(n)`,example:`# Basic usage
...
# At least one match
numbers = [2, 4, 6, 7, 8]
print(any(n % 2 == 1 for n in numbers))  # True (7 is odd)`},{section:`Control Flow`,signature:`if/while truthiness`,description:`Control statements use truthiness. Pythonic code checks truthiness directly, not explicit comparisons.`,complexity:`O(1)`,example:`# PYTHONIC - check truthiness directly
lst = [1, 2, 3]
if lst:  # GOOD
...
if x < 3:
    ...
    result = "large"`},{section:`Advanced Patterns`,signature:`__bool__(self)`,description:`Define custom truthiness for classes. Called by bool() and if statements. Falls back to __len__() if not defined.`,complexity:`O(1)`,example:`# Custom __bool__ method
class Account:
...
print(clean)  # [1, 2, 3]`}],s=[{signature:`List Basics`,description:`Lists are MUTABLE ordered sequences. Create with brackets []. Can modify elements. Use for dynamic collections that need to change.`,complexity:`Concept`,example:`# Create lists with square brackets []
...
5 in lookup          # O(1) each lookup`}],c=[{section:`Ordering`,signature:`list.sort(*, key=None, reverse=False)`,description:`Sorts list IN PLACE. Returns None (not the list!). Stable sort (equal elements keep order).`,complexity:`O(n log n)`,example:`lst = [3, 1, 4, 1, 5]
lst.sort()
print(lst)  # [1, 1, 3, 4, 5]
...
# One-liner
n = 3
[lst[i:i+n] for i in range(0, len(lst), n)]`}],l=[...s,...c],u=[{section:`Fundamentals`,signature:`Tuple Basics`,description:`Tuples are IMMUTABLE ordered sequences. Create with parentheses (). Can contain any types. Remember: tuples CANNOT be modified after creation.`,complexity:`Concept`,example:`# Create tuples with parentheses
my_tuple = (1, 2, 3)
...
# hash(([1, 2],))  # TypeError: unhashable type: 'list'`}],d=[{section:`Fundamentals`,signature:`Dictionary Basics`,description:`Dictionaries store KEY-VALUE pairs. Create with {}. Keys must be hashable (immutable). Values can be anything. Order preserved since Python 3.7.`,complexity:`Concept`,example:`# Create dict with curly braces {}
capitals = {
...
# dict_items([('California', 'Sacramento'), ...])`},{section:`Why & When`,signature:`Why use Dictionary?`,description:`Dicts provide O(1) average key-value lookups. Use when you need fast access by key, counting, caching, or mapping relationships.`,complexity:`Concept`,example:`# Dictionary = Hash Table = O(1) lookup
...
# For small fixed keys, consider namedtuple`},{section:`Creation`,signature:`dict()`,description:`Creates an empty dictionary or converts mappings/iterables to dict.`,complexity:`O(n)`,example:`print(dict())                    # {}
...
d = dict.fromkeys(keys, [])
d['a'].append(1)
print(d)  # {'a': [1], 'b': [1], 'c': [1]} (all same list!)`},{section:`Access`,signature:`dict[key]`,description:`Returns value for key. Raises KeyError if key not found.`,complexity:`O(1) avg`,example:`d = {'name': 'Alice', 'age': 30}
print(d['name'])  # 'Alice'
...
print(groups)  # {'a': ['apple', 'apricot'], 'b': ['banana']}`},{section:`Modification`,signature:`dict[key] = value`,description:`Sets value for key. Adds key if not present.`,complexity:`O(1) avg`,example:`d = {'a': 1}
...
d |= {'b': 99, 'c': 3}
print(d)  # {'a': 1, 'b': 99, 'c': 3}`},{section:`Removal`,signature:`dict.pop(key[, default])`,description:`Removes key and returns its value. Returns default if not found (or raises KeyError).`,complexity:`O(1) avg`,example:`d = {'a': 1, 'b': 2, 'c': 3}
print(d.pop('b'))      # 2
print(d)               # {'a': 1, 'c': 3}
...
d.clear()
print(d)  # {}`},{section:`Views`,signature:`dict.keys()`,description:`Returns a view of the dictionary's keys.`,complexity:`O(1)`,example:`d = {'a': 1, 'b': 2, 'c': 3}
keys = d.keys()
...
for key, value in d.items():
    print(f"{key}: {value}")`},{section:`Membership & Length`,signature:`key in dict`,description:`Returns True if key is in the dictionary.`,complexity:`O(1) avg`,example:`d = {'a': 1, 'b': 2}
print('a' in d)      # True
print('z' in d)      # False
...
print(len(d))  # 3`},{section:`Copying`,signature:`dict.copy()`,description:`Returns a shallow copy of the dictionary.`,complexity:`O(n)`,example:`d = {'a': 1, 'b': [2, 3]}
copy = d.copy()
copy['a'] = 99
...
deep['b'].append(4)
print(d)     # {'a': 1, 'b': [2, 3]} (unchanged!)
print(deep)  # {'a': 1, 'b': [2, 3, 4]}`},{section:`Iteration`,signature:`for key in dict`,description:`Iterates over keys (default iteration).`,complexity:`O(n)`,example:`d = {'a': 1, 'b': 2}
for key in d:
...
for key, value in d.items():
    print(f"{key} = {value}")`},{section:`Specialized Dicts`,signature:`defaultdict(default_factory)`,description:`Dict subclass that provides default values for missing keys.`,complexity:`O(1)`,example:`from collections import defaultdict
...
print(od.popitem(last=False))  # ('b', 2)`},{section:`Common Patterns`,signature:`Invert dictionary`,description:`Swap keys and values. Values must be hashable.`,complexity:`O(n)`,example:`d = {'a': 1, 'b': 2, 'c': 3}
...
# {'a': ['apple', 'apricot'], 'b': ['banana', 'blueberry']}`}],f=[{section:`Why & When`,signature:`Why use Set?`,description:`Sets provide O(1) membership testing and automatic deduplication. Use when you need unique elements or fast "is X in collection" checks.`,complexity:`Concept`,example:`# Set = Hash Table for unique values
...
print(all_valid)  # False ('guest' not allowed)`}];var p=e();function m(e,n){return function(){return(0,p.jsx)(t,{type:e.type,badge:e.badge,color:e.color,description:e.description,intro:e.intro,tip:e.tip,methods:n})}}const h=m(n.string,r),g=m(n.int,i),_=m(n.float,a),v=m(n.bool,o),y=m(n.list,l),b=m(n.tuple,u),x=m(n.dict,d),S=m(n.set,f);export{v as BoolPage,x as DictPage,_ as FloatPage,g as IntPage,y as ListPage,S as SetPage,h as StringPage,b as TuplePage};