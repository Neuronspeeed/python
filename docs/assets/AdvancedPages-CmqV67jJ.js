...
- Use \`timeit\` for micro-benchmarks, \`cProfile\` for whole-program profiling`},concurrency:{type:`Concurrency`,badge:`async`,color:`var(--accent-concurrency)`,description:`Concurrent and parallel programming in Python. Threading, multiprocessing, and async/await.`,intro:`Concurrency and parallelism allow programs to do multiple things at once—but in Python, the approach you choose matters enormously. The GIL (Global Interpreter Lock) in CPython is the critical constraint that determines which concurrency model to use. Understanding threading vs multiprocessing vs async/await—and when to use each—is essential for writing performant Python applications.
...
# Framework/library → comprehensive Sphinx`}],i=[{section:`Comments`,signature:`# comment`,description:`Basic in-file documentation. Python ignores text after #. Best for small-scale implementation notes.`,complexity:`Concept`,example:`x = 10  # inline comment
...
for user in users:
    if not user.active:
        continue`}],a=[{section:`dir Function`,signature:`dir(object)`,description:`Lists all attribute names of an object. A "memory jogger" showing available methods without descriptions.`,complexity:`O(n)`,example:`# List string methods
dir(str)
...
# ['rsplit', 'split', 'splitlines']`}],o=[{section:`Docstrings`,signature:`"""docstring"""`,description:`String at top of module/function/class. Stored in __doc__ attribute. Use triple quotes for multiline.`,complexity:`Concept`,example:`def greet(name):
...
# in Python 3.12+`}],s=[{section:`help & pydoc`,signature:`help(object)`,description:`Interactive documentation viewer. Shows docstrings, method signatures, and type info.`,complexity:`Concept`,example:`# Get help on a function
help(len)
...
$ python -m pydoc -p 8080
# Then open http://localhost:8080`}],c=[{section:`External Tools`,signature:`Sphinx`,description:`Third-party tool for rich documentation. Generates HTML/PDF from reStructuredText or Markdown.`,complexity:`Concept`,example:`# Install Sphinx
$ pip install sphinx
...
# themes, and hosting on ReadTheDocs`},{section:`External Tools`,signature:`Type Hints as Docs`,description:`Type annotations serve as inline documentation. Tools like mypy validate them.`,complexity:`Concept`,example:`def process(
    ...
    """
    pass`}],l=[...r,...i,...a,...o,...s,...c],u=[{section:`Why & When`,signature:`When to split code into modules`,description:`Split when: file > 300 lines, distinct responsibilities, reusable across projects, or team collaboration needs.`,complexity:`Concept`,example:`# MONOLITH - hard to navigate, test, reuse
# app.py (1500 lines)
def connect_db(): ...
def send_email(): ...
def process_payment(): ...
...
# Run entire package:
# python -m mypackage
# Example: python -m http.server 8000`}],d=[{section:`Why & When`,signature:`Dynamic imports vs static imports`,description:`Static imports (import x): compile-time, fast, analyzable. Dynamic imports (importlib): runtime, flexible, plugins. Use static 99% of the time.`,complexity:`Concept`,example:`# STATIC - compile-time, normal usage
import json
import requests
...
# - Library: 1-2 levels
# - If > 4 levels, rethink structure`}],f=[{section:`Program Structure`,signature:`Main script + modules`,description:`Programs have one main script (entry point) and library modules providing tools.`,complexity:`Concept`,example:`# project/
#   main.py          ← entry point
#   utils/
#     __init__.py
#     helpers.py
...
# my-package/ will cause SyntaxError on import`}],p=[{section:`Namespaces`,signature:`Module as namespace`,description:`A module is a package of variable names. Top-level assignments become module attributes.`,complexity:`Concept`,example:`# mymodule.py
...
print(mod1.x)  # 10
print(mod2.x)  # 99
# Qualification keeps them separate`}],m=[{section:`Import Variants`,signature:`from module import *`,description:`Copies all top-level names from module. Avoid in production—pollutes namespace.`,complexity:`O(n)`,example:`from math import *
print(sqrt(16))  # 4.0
print(pi)        # 3.14159...
...
from mymodule import _internal  # works`}],h=[{section:`Reloading`,signature:`importlib.reload()`,description:`Re-run module code to pick up changes. Requires already-loaded module object.`,complexity:`O(n)`,example:`from importlib import reload
import mymodule
...
def transitive_reload(module, visited=None):
    ...
    reload(module)`}],g=[{section:`Dynamic Imports`,signature:`importlib.import_module()`,description:`Import module by name string at runtime. Foundation for plugin architectures.`,complexity:`O(n)`,example:`import importlib
...
handler = importlib.import_module(config["handler"])`}],_=[...d,...f,...p,...m,...h,...g],v=[...u,..._],y=[{signature:`Exceptions vs return codes`,description:`Exceptions: abnormal conditions, can't be ignored. Return codes/None: expected alternatives, can be ignored. Use exceptions when caller shouldn't continue.`,complexity:`Concept`,section:`Why & When`,example:`# RETURN CODES - expected cases
...
try:
    ...
    log_error()`}],b=[{signature:`try: ... except:`,description:`Basic exception handling. Catches all exceptions if no type specified.`,complexity:`O(1)`,section:`Basic Handling`,example:`try:
    x = 1 / 0
except:
...
test()  # Prints "Finally runs!" then returns 1`}],x=[{signature:`raise ExceptionType`,description:`Raise an exception to signal an error.`,complexity:`O(1)`,section:`Raising Exceptions`,example:`def validate_age(age):
    if age < 0:
...
try:
    ...
    print(e.__cause__)       # Original FileNotFoundError`}],S=[{signature:`class CustomError(Exception):`,description:`Create custom exception by inheriting from Exception.`,complexity:`O(1)`,section:`Custom Exceptions`,example:`class ValidationError(Exception):
...
try:
    ...
    print(f"API Error: {e}")`}],C=[{signature:`ValueError`,description:`Raised when function receives correct type but invalid value.`,complexity:`O(1)`,section:`Built-in Exceptions`,example:`int("abc")      # ValueError: invalid literal
int("3.14")     # ValueError
...
# Don't use for data validation in production!`}],w=[...y,...b,...x,...S,...C],T=[{signature:`with statement`,description:`Automatic resource management. Ensures cleanup.`,complexity:`O(1)`,section:`Context Managers`,example:`# File automatically closed
with open("file.txt") as f:
    content = f.read()
...
with open("output.txt", "w") as f:
    with redirect_stdout(f):
        print("Goes to file")`}],E=[{signature:`Context managers vs try/finally`,description:`Context managers (with): for resources needing cleanup (files, locks). try/finally: for one-off cleanup. Prefer with for reusable patterns.`,complexity:`Concept`,section:`Why & When`,example:`# ONE-OFF CLEANUP - try/finally
lock.acquire()
try:
    critical_section()
finally:
...
# - Suppressed: raise X from None`}],D=[{signature:`Nested try/finally (propagation)`,description:`Exceptions bubble up through nested handlers. All finally blocks execute during unwinding.`,complexity:`O(1)`,section:`Nesting & Idioms`,example:`def inner():
    try:
...
try:
    ...
    print(e.log())      # [DB-503] Connection failed | Query: SELECT *`}],O=[{signature:`warnings.warn()`,description:`Issue a warning without stopping execution.`,complexity:`O(1)`,section:`Warnings & Info`,example:`import warnings
...
try:
    ...
    print(f"Line: {exc_tb.tb_lineno}")`}],k=[...T,...E,...D,...O],A=[...w,...k],j=[{signature:`Logging vs print() - when to use each`,description:`print(): development/debugging. logging: production. Logging provides levels, timestamps, file output, filtering. Never use print in production.`,complexity:`Concept`,section:`Why & When`,example:`# PRINT - development only
...
# {"timestamp": "2024-01-15 10:30:45", "level": "INFO", ...}`}],ee=[{signature:`import pdb; pdb.set_trace()`,description:`Start interactive debugger at this point.`,complexity:`O(1)`,section:`Debugging with pdb`,example:`def buggy_function(x, y):
    result = x + y
...
# Run with:
# DEBUG=1 python script.py`}],M=[...j,...ee],N=[{signature:`Threads vs Processes vs Async - decision tree`,description:`I/O-bound → async (fastest) or threads. CPU-bound → multiprocessing. Simple I/O → threads. Complex async logic → async/await.`,complexity:`Concept`,section:`Why & When`,example:`# I/O-BOUND (network, disk, database)
...
# - Tasks < 100ms
# - Need shared memory
# - Simple concurrency`}],P=[{signature:`import threading`,description:`Thread-based parallelism. Good for I/O-bound tasks. Limited by GIL for CPU-bound.`,complexity:`O(1)`,section:`Threading Basics`,example:`import threading
import time
...
q.join()  # Wait until all items processed`}],F=[{signature:`ThreadPoolExecutor`,description:`Pool of threads for concurrent execution.`,complexity:`O(1)`,section:`ThreadPoolExecutor`,example:`from concurrent.futures import ThreadPoolExecutor
import time
...
with ThreadPoolExecutor(max_workers=3) as executor:
    ...
    for future in as_completed(futures):
        n = futures[future]
        result = future.result()
        print(f"process({n}) = {result}")`}],I=[{signature:`import multiprocessing`,description:`Process-based parallelism. Bypasses GIL. Best for CPU-bound tasks.`,complexity:`O(1)`,section:`Multiprocessing`,example:`import multiprocessing
...
if __name__ == '__main__':
    ...
    print(list(arr))      # [5.0, 6.0, 7.0]`}],L=[...N,...P,...F,...I],R=[{signature:`Async/await vs Threads - when to choose async`,description:`Async wins: many (100+) I/O tasks, web servers, API clients. Threads win: few (<10) I/O tasks, legacy code, simpler mental model.`,complexity:`Concept`,section:`Why & When`,example:`# ASYNC WINS - many concurrent I/O operations
import asyncio
import aiohttp
...
stats.print_stats(10)`}],z=[{signature:`async def`,description:`Define coroutine function. Must be awaited.`,complexity:`O(1)`,section:`Async/Await`,example:`import asyncio
...
asyncio.run(main())`}],B=[{signature:`async with`,description:`Async context manager for resources.`,complexity:`O(1)`,section:`Async Context & Iterators`,example:`import asyncio
...
asyncio.run(main())`}],V=[{signature:`aiohttp (async HTTP)`,description:`Popular async HTTP client library.`,complexity:`O(1)`,section:`Async HTTP`,example:`# pip install aiohttp
import asyncio
import aiohttp
...
# asyncio.run(main())`}],H=[{signature:`Threading vs Multiprocessing vs Async`,description:`Choose the right tool for the job.`,complexity:`O(1)`,section:`Best Practices`,example:`# THREADING - Good for:
...
asyncio.run(main())`}],U=[...R,...z,...B,...V,...H],W=[...L,...U],G=[{section:`Why & When`,signature:`Text mode vs Binary mode - when to use each`,description:`Text mode: human-readable files (.txt, .csv, .json). Binary mode: everything else (.jpg, .mp3, .pdf). Wrong mode = corruption.`,complexity:`Concept`,example:`# TEXT MODE - human-readable content
with open("data.txt", "r", encoding="utf-8") as f:
...
# - Read before write (same file)`}],K=[{section:`File Basics`,signature:`What is a file?`,description:`A file is a sequence of bytes (integers 0-255). Must be decoded/encoded to interpret contents. Python handles encoding/decoding for you.`,complexity:`Concept`,example:`# Files are sequences of bytes
...
# Rarely a problem in practice!`}],q=[{section:`Opening Files`,signature:`open(path, mode, encoding)`,description:`Opens file and returns file object. Specify mode ("r", "w", "a") and encoding. Always close with .close() or use with statement.`,complexity:`O(1)`,example:`# Basic file opening
...
content = file.read()
file.close()  # What if exception before this?`}],J=[{section:`Reading Files`,signature:`file.read()`,description:`Reads entire file as single string. Lines separated by \\n. Good for small files.`,complexity:`O(n)`,example:`# Read entire file into string
with open("data.txt", "r", encoding="utf-8") as file:
    content = file.read()
    print(content)
...
if path.exists():
    with open(path, "r") as file:
        content = file.read()`}],Y=[{section:`Writing Files`,signature:`file.write(text)`,description:`Writes string to file. Must open in "w" (overwrite) or "a" (append) mode. Returns number of characters written.`,complexity:`O(n)`,example:`# Write mode OVERWRITES file!
...
for line in bad_lines:
    file.write(line + "\\n")`}],X=[{section:`pathlib`,signature:`Path(string)`,description:`Creates Path object from string. Cross-platform file path handling. Use / operator to join paths.`,complexity:`O(1)`,example:`from pathlib import Path
...
dir_path.mkdir(exist_ok=True)`}],Z=[...[...G,...K,...q,...J,...Y,...X]];var Q=e();function $(e,n){return function(){return(0,Q.jsx)(t,{type:e.type,badge:e.badge,color:e.color,description:e.description,intro:e.intro,methods:n})}}const te=$(n.documentation,l),ne=$(n.modules,v),re=$(n.exceptions,A),ie=$(n.logging,M),ae=$(n.concurrency,W),oe=$(n.fileio,Z);export{ae as ConcurrencyPage,te as DocumentationPage,re as ExceptionsPage,oe as FileIOPage,ie as LoggingPage,ne as ModulesPage};